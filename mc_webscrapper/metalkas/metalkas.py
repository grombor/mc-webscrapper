...
class Metalkas(ScrapperClass):
    ...
    def get_model_name(self, soup) -> str:
        try:
            return soup.select_one("h1.page-title").text
    ...
    def get_shop_price_nett(self, soup) -> int: 
        try:
            ...
            price_gross = extract_digits(price_gross)
            price_nett = int(price_gross / 1.3 )
            return price_nett
        except (IndexError) :
    ...
    def gather_data_from_link(self, link):
        ...
        metalkas.lead_time = self.get_lead_time(soup)
